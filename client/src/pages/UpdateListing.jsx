...
function UpdateListing() {
  ...
  useEffect(() => {
    async function getListing() {
      const res = await axios.get(`/api/listings/getListing/${param.id}`);
  ...
  const submitForm = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`/api/listings/updateListing/${param.id}`, {
        ...formData,
        userRef: userID,
      });
      setLoading(false);
      if (res?.data.status == "success") {
        navigate(`/listing/${res.data.query2._id}`);
      } else {
        console.log(res);
      }
    } catch (error) {
